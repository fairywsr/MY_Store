...
import {upload_icon} from "../../assets/data"
...
function Add() {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      const productData = {name, description, category, price, offerPrice, sizes, popular}
  ...
  return (
    <div className="px-2 sm:px-6 py-12 m-2 h-[97vh] bg-primary overflow-y-scroll w-full lg:w-4/5 rounded-xl">
      <form onSubmit={onSubmitHandler} className="flex flex-col gap-y-3 medium-14">
            ...
            <div className="flex flex-row gap-4">
              <div>
                ...
                <select
                  ...
                  className="max-w-20 px-3 py-2 text-gray-30 ring-1 ring-slate-900/10 bg-white rounded"
                >
                  ...
                  <option value="sportwear">Sportwearr</option>
                </select>
              </div>
            </div>
            <div>
        ...
        <div className="flex gap-2 pt-2">
          {
            Array(4).fill('').map((_, index)=>(
              <label key={index} htmlFor={`image${index}`} className="roundd overflow-hidden">
                <input onChange={(e)=>{
        ...
        <button type="submit" className="btn-dark mt-3 max-w-44 sm:-full rounded">Add Product</button>
      </form>
    </div>
  );
}
